...
export const VIEWPORT_CLASSES: ViewportClass[] = ['desktop', 'tablet', 'mobile'];
...
const MOCK_TRANSACTIONS: MockTransaction[] = [
    { id: 'tx-001', type: 'payment', amount: '100.00', asset: MOCK_ASSET, timestamp: new Date('2024-01-15T10:00:00Z') },
    { id: 'tx-002', type: 'swap',    amount: '50.00',  asset: MOCK_ASSET, timestamp: new Date('2024-01-14T09:00:00Z') },
    { id: 'tx-003', type: 'payment', amount: '200.00', asset: MOCK_ASSET, timestamp: new Date('2024-01-13T08:00:00Z') },
];
 ...
 */
export function deriveLayoutMetadata(viewportClass: ViewportClass): LayoutMetadata {
    const viewport = VIEWPORT_DIMENSIONS[viewportClass];

    switch (viewportClass) {
        case 'desktop':
            return { viewport, viewportClass, containerMaxWidth: 1280, sidebarCollapsed: false, gridColumns: 12 };
        case 'tablet':
            return { viewport, viewportClass, containerMaxWidth: 720,  sidebarCollapsed: true,  gridColumns: 8  };
        case 'mobile':
            return { viewport, viewportClass, containerMaxWidth: 360,  sidebarCollapsed: true,  gridColumns: 4  };
    }
}
